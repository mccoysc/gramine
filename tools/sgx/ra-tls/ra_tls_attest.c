...
#define NUM_SUPPORTED_ALGORITHMS \
    (sizeof(g_supported_algorithms) / sizeof(g_supported_algorithms[0]))
...
static int json_get_string(const char* json, jsmntok_t* tokens, int num_tokens, const char* key,
                           char** out_value) {
...
static int json_get_bool(const char* json, jsmntok_t* tokens, int num_tokens, const char* key,
                         bool* out_value) {
    for (int i = 0; i < num_tokens - 1; i++) {
        if (tokens[i].type == JSMN_STRING) {
            int key_len = tokens[i].end - tokens[i].start;
...
static int parse_json_config(const char* b64_json, cert_config_t* config) {
    ...
    /* Decode base64 */
    ret =
        mbedtls_base64_decode(NULL, 0, &json_len, (const unsigned char*)b64_json, strlen(b64_json));
    if (ret != MBEDTLS_ERR_BASE64_BUFFER_TOO_SMALL) {
        return -EINVAL;
    }
    ...
    json_get_string((const char*)json_buf, tokens, num_tokens, "signature_md",
                    &config->signature_md);
    ...
    json_get_string((const char*)json_buf, tokens, num_tokens, "ca_key_format",
                    &config->ca_key_format);
    json_get_string((const char*)json_buf, tokens, num_tokens, "ca_algorithm",
                    &config->ca_algorithm);
    json_get_string((const char*)json_buf, tokens, num_tokens, "ca_cert_file",
                    &config->ca_cert_file);
    json_get_string((const char*)json_buf, tokens, num_tokens, "ca_cert_format",
                    &config->ca_cert_format);
    json_get_string((const char*)json_buf, tokens, num_tokens, "ca_subject", &config->ca_subject);
    json_get_string((const char*)json_buf, tokens, num_tokens, "ca_not_before",
                    &config->ca_not_before);
    json_get_string((const char*)json_buf, tokens, num_tokens, "ca_not_after",
                    &config->ca_not_after);
        ...
        if (ec) {
            ...
            switch (grp_id) {
                ...
                case MBEDTLS_ECP_DP_SECP224K1:
                    return MBEDTLS_MD_SHA224; /* P-224 -> SHA-224 */
...
static int generate_x509(mbedtls_pk_context* pk, const uint8_t* quote, size_t quote_size,
                         ...
                         mbedtls_x509write_cert* writecrt, mbedtls_pk_context* ca_key,
                         const char* ca_subject_name, const char* subject_name,
                         const char* not_before, const char* not_after, mbedtls_md_type_t md_type,
                         bool is_ca) {
    int ret;
    ...
    if (mbedtls_pk_get_type(pk) == MBEDTLS_PK_ECKEY ||
        ...
        if (ec) {
            ...
            if (grp_id == MBEDTLS_ECP_DP_SECP256R1)
                curve_name = "P-256";
            else if (grp_id == MBEDTLS_ECP_DP_SECP192R1)
                curve_name = "P-192";
    ...
    if (ca_key) {
        ...
        if (mbedtls_pk_get_type(ca_key) == MBEDTLS_PK_ECKEY ||
            mbedtls_pk_get_type(ca_key) == MBEDTLS_PK_ECDSA) {
    ...
    if (not_before) {
        ...
        cert_timestamp_not_before =
            strdup(getenv(RA_TLS_CERT_TIMESTAMP_NOT_BEFORE) ?: CERT_TIMESTAMP_NOT_BEFORE_DEFAULT);
    }
    ...
    if (not_after) {
        ...
        cert_timestamp_not_after =
            strdup(getenv(RA_TLS_CERT_TIMESTAMP_NOT_AFTER) ?: CERT_TIMESTAMP_NOT_AFTER_DEFAULT);
    }
...
static int combine_quote_and_claims_in_evidence(uint8_t* quote, size_t quote_size, uint8_t* claims,
...
static int generate_ca_certificate(mbedtls_pk_context* ca_key, mbedtls_x509_crt* ca_crt,
    ...
    ret =
        mbedtls_x509write_crt_set_basic_constraints(&ca_writecrt, /*is_ca=*/1, /*max_pathlen=*/-1);
    if (ret < 0)
        goto out;

    /* Set key usage: KEY_CERT_SIGN | CRL_SIGN */
    ret = mbedtls_x509write_crt_set_key_usage(
        &ca_writecrt, MBEDTLS_X509_KU_KEY_CERT_SIGN | MBEDTLS_X509_KU_CRL_SIGN);
    if (ret < 0)
        goto out;
...
static int create_x509(mbedtls_pk_context* pk, mbedtls_x509write_cert* writecrt,
                       mbedtls_pk_context* ca_key, const char* ca_subject, const char* subject,
                       const char* not_before, const char* not_after, mbedtls_md_type_t md_type,
                       bool is_ca) {
    int ret;

    /*
     * We put both "legacy Gramine" OID with plain SGX quote as well as standardized TCG DICE
     * "tagged evidence" OID with CBOR-formatted SGX quote into the RA-TLS X.509 cert. This is for
     * keeping backward compatibility at the price of a larger size of the resulting cert.
     */
    uint8_t* quote    = NULL;
    uint8_t* evidence = NULL;
    ...
    ret = generate_x509(pk, quote, quote_size, evidence, evidence_size, writecrt, ca_key,
                        ca_subject, subject, not_before, not_after, md_type, is_ca);
out:
    free(quote);
    free(evidence);
    return ret;
...
static int create_key_and_crt_legacy(mbedtls_pk_context* key, mbedtls_x509_crt* crt,
    ...
    ret = mbedtls_ecp_gen_key(MBEDTLS_ECP_DP_SECP384R1, mbedtls_pk_ec(*key),
                              mbedtls_ctr_drbg_random, &ctr_drbg);
    if (ret < 0)
        goto out;
    ...
    ret = create_x509(key, &writecrt, /*ca_key=*/NULL, /*ca_subject=*/NULL,
                      /*subject=*/NULL, /*not_before=*/NULL, /*not_after=*/NULL, MBEDTLS_MD_SHA256,
                      /*is_ca=*/false);
    if (ret < 0)
        goto out;
...
static int create_key_and_crt_enhanced(mbedtls_pk_context* key, mbedtls_x509_crt* crt,
                                       uint8_t** crt_der, size_t* crt_der_size) {
    int ret = 0;
    ...
    if (use_json && json_config.ca_key_file) {
        ...
        if (fd >= 0) {
            ...
            printf(
                "RA-TLS: CA key file does not exist (will be treated as path specification): %s\n",
                json_config.ca_key_file);
        }
    }
    ...
    if (use_json && json_config.ca_cert_file) {
        ...
        if (fd >= 0) {
            ...
            printf(
                "RA-TLS: CA cert file does not exist (will be treated as path specification): %s\n",
                json_config.ca_cert_file);
        }
    }
    ...
    bool use_ca_logic = (use_json && (json_config.ca_key_file || json_config.ca_cert_file ||
    ...
    if (use_ca_logic) {
        ...
        if (load_existing_ca) {
            ...
            bool is_pem =
                !json_config.ca_key_format || strcasecmp(json_config.ca_key_format, "pem") == 0;
            if (is_pem) {
                ret =
                    mbedtls_pk_parse_keyfile(&ca_key_ctx, json_config.ca_key_file,
                                             /*password=*/NULL, mbedtls_ctr_drbg_random, &ctr_drbg);
            } else {
                /* DER format - read file and parse */
                uint8_t ca_key_buf[8192];
                ssize_t ca_key_len =
                    read_file(json_config.ca_key_file, ca_key_buf, sizeof(ca_key_buf));
                if (ca_key_len < 0) {
                ...
                ret = mbedtls_pk_parse_key(&ca_key_ctx, ca_key_buf, ca_key_len, /*password=*/NULL,
            ...
            printf("RA-TLS: Loading existing CA certificate from file: %s\n",
                   json_config.ca_cert_file);
            ret =
                load_and_verify_ca_certificate(json_config.ca_cert_file, json_config.ca_cert_format,
            ...
            if (ret > 0) {
                ...
                ca_subject_ptr =
                    json_config.ca_subject ? json_config.ca_subject : CERT_SUBJECT_NAME_VALUES;
            ...
            if (ca_cert_file_exists && !ca_key_file_exists) {
                printf(
                    "RA-TLS: CA cert exists but CA key doesn't - reading existing CA cert info\n");
                mbedtls_x509_crt temp_ca_crt;
                mbedtls_x509_crt_init(&temp_ca_crt);

                bool is_pem = !json_config.ca_cert_format ||
                              strcasecmp(json_config.ca_cert_format, "pem") == 0;
                int temp_ret;
                if (is_pem) {
                    temp_ret = mbedtls_x509_crt_parse_file(&temp_ca_crt, json_config.ca_cert_file);
                } else {
                    uint8_t temp_cert_buf[8192];
                    ssize_t temp_cert_len =
                        read_file(json_config.ca_cert_file, temp_cert_buf, sizeof(temp_cert_buf));
                    if (temp_cert_len > 0) {
                        temp_ret =
                            mbedtls_x509_crt_parse_der(&temp_ca_crt, temp_cert_buf, temp_cert_len);
                    } else {
                        temp_ret = -1;
                    }
                }
                ...
                if (temp_ret == 0) {
                    ...
                    char subject_buf[256];
                    temp_ret = mbedtls_x509_dn_gets(subject_buf, sizeof(subject_buf),
                                                    &temp_ca_crt.subject);
                    if (temp_ret > 0) {
                        ca_subject_from_cert = strdup(subject_buf);
                    ...
                    snprintf(not_before_buf, sizeof(not_before_buf), "%04d%02d%02d%02d%02d%02d",
                             temp_ca_crt.valid_from.year, temp_ca_crt.valid_from.mon,
                             temp_ca_crt.valid_from.day, temp_ca_crt.valid_from.hour,
                             temp_ca_crt.valid_from.min, temp_ca_crt.valid_from.sec);
                    snprintf(not_after_buf, sizeof(not_after_buf), "%04d%02d%02d%02d%02d%02d",
                             temp_ca_crt.valid_to.year, temp_ca_crt.valid_to.mon,
                             temp_ca_crt.valid_to.day, temp_ca_crt.valid_to.hour,
                             temp_ca_crt.valid_to.min, temp_ca_crt.valid_to.sec);
            ...
            if (ca_key_file_exists) {
                ...
                bool is_pem =
                    !json_config.ca_key_format || strcasecmp(json_config.ca_key_format, "pem") == 0;
                if (is_pem) {
                    ret = mbedtls_pk_parse_keyfile(&ca_key_ctx, json_config.ca_key_file,
                                                   /*password=*/NULL, mbedtls_ctr_drbg_random,
                                                   &ctr_drbg);
                } else {
                    uint8_t ca_key_buf[8192];
                    ssize_t ca_key_len =
                    ...
                    ret =
                        mbedtls_pk_parse_key(&ca_key_ctx, ca_key_buf, ca_key_len, /*password=*/NULL,
                                             0, mbedtls_ctr_drbg_random, &ctr_drbg);
                }
                if (ret < 0) {
                    log_mbedtls_error("Load CA key from file", ret);
                ...
                if (json_config.ca_algorithm) {
                    ...
                    for (size_t i = 0; i < NUM_SUPPORTED_ALGORITHMS; i++) {
                        if (strcasecmp(g_supported_algorithms[i].name, json_config.ca_algorithm) ==
                            0) {
                    ...
                    if (!ca_algo_config) {
                        printf(
                            "RA-TLS: WARNING: CA algorithm '%s' not found, will use same algorithm "
                            "as user key\n",
                            json_config.ca_algorithm);
                    } else {
                ...
                if (!ca_algo_config) {
                    printf(
                        "RA-TLS: No valid ca_algorithm specified, using same algorithm as user "
                        "key\n");
                    ...
                    if (user_pk_type == MBEDTLS_PK_ECKEY || user_pk_type == MBEDTLS_PK_ECDSA) {
                        ...
                        ret =
                            mbedtls_rsa_gen_key(mbedtls_pk_rsa(ca_key_ctx), mbedtls_ctr_drbg_random,
                                                &ctr_drbg, user_key_bits, 65537);
                        if (ret < 0) {
                    ...
                    ret = mbedtls_pk_setup(&ca_key_ctx,
                                           mbedtls_pk_info_from_type(ca_algo_config->pk_type));
                    if (ret < 0) {
                        log_mbedtls_error("CA PK setup", ret);
                        mbedtls_pk_free(&ca_key_ctx);
                        mbedtls_x509_crt_free(ca_crt_heap);
                    ...
                    if (ca_algo_config->pk_type == MBEDTLS_PK_ECKEY) {
                        ...
                        ret = mbedtls_ecp_gen_key(ca_algo_config->params.ecp_group_id,
                                                  mbedtls_pk_ec(ca_key_ctx),
                                                  mbedtls_ctr_drbg_random, &ctr_drbg);
                        if (ret < 0) {
                            log_mbedtls_error("CA EC key generation", ret);
                            mbedtls_pk_free(&ca_key_ctx);
                        ...
                        ret = mbedtls_rsa_gen_key(mbedtls_pk_rsa(ca_key_ctx),
                                                  mbedtls_ctr_drbg_random, &ctr_drbg,
                ...
                if (json_config.ca_key_file) {
                    ...
                    if (ret < 0) {
                        printf(
                            "RA-TLS: WARNING: Failed to write CA private key file (continuing "
                            "anyway)\n");
                    } else {
            ...
            ret = generate_ca_certificate(&ca_key_ctx, ca_crt_heap, ca_subject_to_use,
                                          ca_not_before_to_use, ca_not_after_to_use, ca_md_type,
                                          &ctr_drbg);
            if (ret < 0) {
            ...
            if (json_config.ca_cert_file) {
                ...
                if (ca_key_dir && ca_key_basename) {
                    ...
                    snprintf(ca_cert_path, sizeof(ca_cert_path), "%s/crt.%s.crt", ca_key_dir,
                             ca_key_basename);
        ...
        if (!json_config.signature_md) {
            ...
            if (recommended_md != md_type) {
                printf(
                    "RA-TLS: WARNING: User-specified signature_md=%s may be incompatible with user "
                    "key\n",
                    get_md_name(md_type));
                ...
                printf(
                    "RA-TLS: WARNING: To fix: either remove 'signature_md' from JSON (auto-detect) "
                    "or set it to '%s'\n",
                    get_md_name(recommended_md));
            }
        }
    ...
    if (ca_was_generated && ca_crt_heap && ca_crt_heap->raw.p && ca_crt_heap->raw.len > 0) {
        ...
        if (ca_cert_file_path) {
            ...
            if (ca_key_dir && ca_key_basename) {
                snprintf(ca_path_buf, sizeof(ca_path_buf), "%s/crt.%s.crt", ca_key_dir,
                         ca_key_basename);
                snprintf(ca_path_fallback, sizeof(ca_path_fallback), "/tmp/crt.%s.crt",
                         ca_key_basename);
            } else {
...
static int create_key_and_crt(mbedtls_pk_context* key, mbedtls_x509_crt* crt, uint8_t** crt_der,
    ...
    if (using_enhanced) {
        printf(
            "RA-TLS: Enhanced configuration detected, attempting enhanced certificate "
            "generation\n");
